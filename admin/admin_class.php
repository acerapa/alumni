...
Class Action {
	...
	function delete_alumni(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM survey where id = ".$id);
		if($delete)
			return 1;
	}
	...
	function save_graduates() {
		extract($_POST);
		$statement = "INSERT INTO graduates (batch_year, graduates) VALUES (?, ?)";
		if (isset($id)) {
			$statement = "UPDATE graduates SET graduates=? WHERE id=$id";
			$stmt = $this->db->prepare($statement);
			$stmt->bind_param("i", $graduates);
		} else {
			// check for duplicates
			$grads = $this->db->query("SELECT * FROM graduates WHERE batch_year=$batch_year");
			$row = mysqli_fetch_assoc($grads);
			if ($grads->num_rows > 0) {
				return "Duplicate Entry!!";
			}
			$stmt = $this->db->prepare($statement);
			$stmt->bind_param("si", $batch_year, $graduates);
		}
		$save = $stmt->execute();
		if ($save) {
			return 1;
	...
	function delete_graduate() {
		extract($_POST);
		$stmt = $this->db->prepare("DELETE FROM graduates where id=?");
